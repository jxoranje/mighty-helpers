...
function formatAssignedDate(value: string | null) {
  return value ? `Assigned for ${value}` : "No date assigned";
}
...
export default function ApprovalsPage() {
    ...
    if (nextStatus === "approved") {
      ...
      setMessage(
        result
          ...
          : "Chore approved and stars awarded."
      );
      setUpdatingId(null);
      return;
    }
  ...
  const kidsWaitingCount = useMemo(
    () => new Set(items.map((item) => item.kid_id)).size,
    [items]
  );
  const withNotesCount = useMemo(
    () => items.filter((item) => Boolean(item.notes?.trim())).length,
    [items]
  );
      ...
      <main className="min-h-screen bg-[var(--background)] px-4 py-5 text-[var(--foreground)] sm:px-6 sm:py-8">
        <div className="mx-auto max-w-xl">
          <section className="rounded-[2rem] border border-[var(--border-soft)] bg-[var(--surface)] p-8 text-center shadow-[0_20px_60px_rgba(33,53,85,0.12)]">
            ...
            <p className="mt-3 text-sm leading-6 text-[var(--muted)]">Please log in to review completed chores for your household.</p>
            <div className="mt-6 flex flex-wrap justify-center gap-3">
              <Link href="/login" className="inline-flex min-h-11 items-center justify-center rounded-full bg-[var(--accent-strong)] px-5 py-3 text-sm font-semibold text-black shadow-sm hover:bg-[var(--accent)]">Log in to your household</Link>
              <Link href="/" className="inline-flex min-h-11 items-center justify-center rounded-full border border-[var(--border-strong)] bg-white px-5 py-3 text-sm font-medium text-[var(--foreground)] hover:bg-[var(--panel-soft)]">Back to home</Link>
            </div>
          </section>
        </div>
      </main>
        ...
        <section className="relative overflow-hidden rounded-[2rem] border border-[var(--border-soft)] bg-[var(--surface)] shadow-[0_20px_60px_rgba(33,53,85,0.12)]">
          ...
          <div className="relative p-5 sm:p-8 md:p-10">
            <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
              <button type="button" onClick={() => router.push("/")} className="inline-flex min-h-11 items-center justify-center rounded-full border border-[var(--border-strong)] bg-white/85 px-4 py-2 text-sm font-medium text-[var(--foreground)] shadow-sm backdrop-blur transition-transform hover:-translate-y-0.5 hover:bg-white">← Back to home</button>
              <button type="button" onClick={() => router.push("/dashboard")} className="inline-flex min-h-11 items-center justify-center rounded-full border border-[var(--border-strong)] bg-white/85 px-4 py-2 text-sm font-medium text-[var(--foreground)] shadow-sm backdrop-blur transition-transform hover:-translate-y-0.5 hover:bg-white">Manage your household</button>
            </div>

            <div className="mt-8 grid gap-5 lg:grid-cols-[1.2fr_0.8fr]">
              <div>
                <div className="inline-flex rounded-full bg-[var(--accent-soft)] px-4 py-2 text-sm font-medium text-[var(--accent-strong)] shadow-sm">Approval queue</div>
                <h1 className="mt-5 font-[family:var(--font-display)] text-4xl leading-tight tracking-[-0.04em] text-[var(--foreground)] sm:text-6xl">Review finished chores and keep stars moving</h1>
                <p className="mt-4 max-w-2xl text-base leading-8 text-[var(--muted)] sm:text-lg">Approve completed chores, send items back when needed, and keep the reward loop clean.</p>
              </div>

              <div className="grid gap-3 sm:grid-cols-3 lg:grid-cols-1">
                <StatCard label="Waiting now" value={pendingCount} />
                <StatCard label="Kids waiting" value={kidsWaitingCount} />
                <StatCard label="With notes" value={withNotesCount} stars />
              </div>
            </div>
            ...
            <section className="mt-8 rounded-[1.85rem] border border-[var(--border-soft)] bg-white/74 p-5 shadow-sm backdrop-blur sm:p-6">
              <div className="flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between">
                <div>
                  <p className="text-xs font-semibold uppercase tracking-[0.16em] text-[var(--muted-strong)]">Pending chores</p>
                  <h2 className="mt-2 text-2xl font-semibold tracking-tight text-[var(--foreground)]">Ready for review</h2>
                </div>
                <p className="text-sm text-[var(--muted)]">Only chores marked done appear here.</p>
              </div>

              {items.length === 0 ? (
                <div className="mt-5 rounded-[1.5rem] border border-[var(--border-soft)] bg-[var(--panel-soft)] px-5 py-6 text-sm text-[var(--muted)]">No chores are waiting for approval right now.</div>
              ) : (
                <div className="mt-5 space-y-4">
                  {items.map((item, index) => {
                    ...
                    return (
                      <article key={item.id} className={`rounded-[1.6rem] border p-5 shadow-sm transition-transform hover:-translate-y-0.5 ${tone.card}`}>
                        <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
                          <div className="min-w-0">
                            <div className="flex flex-wrap gap-2">
                              <span className={`rounded-full border px-3 py-1 text-xs font-semibold ${tone.pill}`}>{item.kid_name || "Unknown kid"}</span>
                              <span className={`rounded-full border px-3 py-1 text-xs font-semibold ${tone.pill}`}>{item.chore_title || "Untitled chore"}</span>
                              <span className={`rounded-full border px-3 py-1 text-xs font-semibold ${tone.action}`}>Done</span>
                            </div>
                            <h3 className="mt-4 text-2xl font-semibold tracking-tight text-[var(--foreground)]">{item.kid_name} completed {item.chore_title}</h3>
                            <p className="mt-3 text-sm leading-6 text-[var(--foreground-soft)] sm:text-base">{formatAssignedDate(item.assigned_for_date)}</p>
                            {item.notes && <div className="mt-4 rounded-[1.2rem] border border-[var(--border-soft)] bg-white/78 px-4 py-3"><p className="text-xs font-semibold uppercase tracking-[0.16em] text-[var(--muted-strong)]">Notes</p><p className="mt-2 text-sm leading-6 text-[var(--foreground-soft)]">{item.notes}</p></div>}
                          </div>
                          <div className="flex flex-col gap-2 sm:flex-row lg:min-w-[280px] lg:justify-end">
                            <button type="button" disabled={isUpdating} onClick={() => updateStatus(item.id, "approved")} className="inline-flex min-h-12 items-center justify-center rounded-full bg-[var(--accent-strong)] px-5 py-3 text-sm font-semibold text-black shadow-[0_16px_35px_rgba(66,109,163,0.28)] transition-transform hover:-translate-y-0.5 hover:bg-[var(--accent)] disabled:cursor-not-allowed disabled:opacity-60">{isUpdating ? "Saving..." : "Approve"}</button>
                            <button type="button" disabled={isUpdating} onClick={() => updateStatus(item.id, "assigned")} className="inline-flex min-h-12 items-center justify-center rounded-full border border-[rgba(190,84,84,0.20)] bg-white/85 px-5 py-3 text-sm font-semibold text-[rgb(140,62,62)] shadow-sm transition-transform hover:-translate-y-0.5 hover:bg-[rgba(255,240,240,0.95)] disabled:cursor-not-allowed disabled:opacity-60">{isUpdating ? "Saving..." : "Reject"}</button>
                          </div>
                        </div>
                      </article>
                    );
...
function StatCard({ label, value, stars = false }: { label: string; value: number; stars?: boolean }) {
  return (
    <div className={`rounded-[1.5rem] border p-4 shadow-sm ${stars ? "border-[var(--star-border)] bg-[linear-gradient(180deg,_#fffaf0_0%,_#fff2d6_100%)]" : "border-[var(--border-soft)] bg-white/72 backdrop-blur"}`}>
      <p className={`text-xs font-semibold uppercase tracking-[0.16em] ${stars ? "text-[var(--star-text)]" : "text-[var(--muted-strong)]"}`}>{label}</p>
